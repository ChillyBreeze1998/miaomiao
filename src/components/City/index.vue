  ...
  <div class="city_body">
    ...
    <div class="city_list">
      <div class="city_hot">
        <h2>热门城市</h2>
        <ul class="clearfix">
          <li v-for="item in hotList" :key="item">{{item.nm}}</li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    ...
    formatCityList(cities) {
      ...
      for (let index = 0; index < cities.length; index++) {
        // 把每个列表的第一个拼音的字母保存到这个变量里 转换为大写
        var firstLetter = cities[index].py.substring(0, 1).toUpperCase();
        // 如果有tocom这个函数的值则新添加
        if (toCom(firstLetter)) {
          cityList.push({
            index: firstLetter,
            list: [{ nm: cities[index].nm, id: cities[index].id }]
          });
      ...
      function toCom(firstLetter) {
        ...
        for (let index = 0; index < cityList.length; index++) {
          // 如果比对成功的话就停止继续比对 也就是return false
          if (cityList[index].index === firstLetter) {
            // console.log(cityList[index].index, firstLetter);
            return false;
          }
        }
        return true;
